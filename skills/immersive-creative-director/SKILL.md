...
1. Before substantive questions, concepts, critique, or planning, read every mandatory core file completely to EOF in this order: [orchestration.md](references/orchestration.md), [preferences.md](references/preferences.md), [experience-contract.md](references/experience-contract.md), [evidence-library.md](references/evidence-library.md), [visual-reference-manifest.md](references/visual-reference-manifest.md), [reference-corpus.md](references/reference-corpus.md), [doctrine.md](references/doctrine.md), [motion-vocabulary.md](references/motion-vocabulary.md), [production.md](references/production.md), [previsualization.md](references/previsualization.md), [quality-gates.md](references/quality-gates.md), and [evolving-the-library.md](references/evolving-the-library.md). A search hit, excerpt, prior memory, summary, or earlier invocation is not the current invocation's read.
2. Run `python3 scripts/validate_visual_corpus.py`. Then use `view_image` to inspect all 15 mandatory overview sheets named in `visual-reference-manifest.md`. Build the required **Visual Grounding Ledger** and complete every reference row. The written corpus and packaged images form one evidence system; prose-only grounding is protocol failure.
3. Complete the corpus-grounding protocol in orchestration, including recognition of every user-named reference and its linked written and visual process evidence.
4. Classify the mode. Mode changes which actions run; it never changes which core knowledge is loaded.
5. Maintain an internal protocol ledger containing every core file and required workflow, its owner, current step, direct completion evidence, and verdict: `required`, `running`, `complete`, `deferred`, or `blocked`.
6. Complete the mode's mandatory intake in orchestration before concept, critique, or production output. Direction and Transformation cannot generate territories until the full Grilling + Domain Modeling frontier is empty and the user confirms the shared understanding.
...
8. Run the bundled experience-contract validator with the required phase. When continuity-dependent generated motion applies, also create the still-first previsualization manifest and run its validator at the state-board, motion, and build boundaries defined in [previsualization.md](references/previsualization.md). Exit code zero is mandatory at every applicable boundary; self-assessment cannot replace it.
9. Before advancing phases, verify every required protocol, visual-grounding, and contract row is `complete` or `strong`. If a mandatory core file or overview cannot be read, `view_image` or a required validator is unavailable, copy lacks legal provenance, or any canonical responsibility is unmapped, report it and stop at the dependent boundary. Continue only unaffected, reversible work.
